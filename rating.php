...
if (isset($_POST['submit_team'])) {
    ...
    $nom_chef     = mysqli_real_escape_string($link, $_POST['leader_name']);
    $research_area = mysqli_real_escape_string($link, $_POST['research_area']);
    $team_number  = mysqli_real_escape_string($link, $_POST['team_number']);
    ...
    if ($res_lab->num_rows > 0) {
        ...
        $check = $link->query("SELECT * FROM teams WHERE name='$nom_equipe' AND laboratory_id='$laboratory_id'");
        if (mysqli_num_rows($check) > 0) {
            array_push($error, "Cette équipe existe déjà pour ce laboratoire.");
        } else {
            $query = "INSERT INTO teams (name, laboratory_id, leader_name, research_area, team_number) 
                      VALUES ('$nom_equipe', '$laboratory_id', '$nom_chef', '$research_area', '$team_number')";
            if (mysqli_query($link, $query)) {
                $success = "Équipe ajoutée avec succès !";
            } else {
...
<head>
    ...
    <title>Team Management | Academic</title>
        ...
        <div class="flex w-full items-center justify-between pb-4">
            <div class="w-full">
                <div class="bg-emerald-600 p-6 rounded-lg">
                    <h4 class="text-white text-xl font-bold">Gestion des Équipes</h4>
                    <p class="text-white opacity-80">Ajouter une nouvelle équipe de recherche</p>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-lg p-8 mt-4 shadow-xl">
                ...
                <form action="rating.php" method="POST">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block mb-2 font-bold text-gray-700">Nom de l'Équipe</label>
                            <input type="text" name="team_name" placeholder="Ex: Équipe IA" required>
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block mb-2 font-bold text-gray-700">Chef d'Équipe</label>
                            <input type="text" name="leader_name" placeholder="Nom du chef d'équipe">
                        </div>
                        <div>
                            <label class="block mb-2 font-bold text-gray-700">Numéro d'Équipe</label>
                            <input type="text" name="team_number" placeholder="Ex: T01">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block mb-2 font-bold text-gray-700">Domaine de Recherche</label>
                            <input type="text" name="research_area" placeholder="Ex: Machine Learning, Réseaux...">
                        </div>
                    </div>
                    <div class="flex justify-end gap-4 mt-8">
                        <button type="reset" class="bg-gray-200 text-gray-700 px-6 py-2 rounded-lg font-bold hover:bg-gray-300">Annuler</button>
                        <button type="submit" name="submit_team" class="bg-emerald-600 text-white px-8 py-2 rounded-lg font-bold hover:bg-emerald-700">Enregistrer</button>
                    </div>
                </form>
            </div>
        </div>
